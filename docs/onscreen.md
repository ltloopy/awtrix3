...
| `TIMER` | Configure the Timer app. Submenu of five items in order: `BUZZER` (Off/End/Countdown), `CDOWN` (countdown beep window, 0–30 s), `FINISH` (auto-clear/hold/re-alert), `CLEAR` (auto-clear delay, 1–300 s), `ALERT` (re-alert cadence, 5–300 s). In the submenu: left/right cycles enum values or adjusts the number (step 5 for `CLEAR`/`ALERT`, step 1 for `CDOWN` — see [ADR 0003](adr/0003-timer-tuning-knobs-on-device.md) for these magnitudes and why); short-press middle advances to the next field; long-press middle saves and exits. Mode changes (`BUZZER`/`FINISH`) apply and publish to MQTT/HA immediately on each press; numeric changes commit on long-press save. |
