...
| `TIMER` | Configure the Timer app. A **drill-in list** consistent with the rest of the menu: left/right walks the named items (and **wraps**), a short press drills into the highlighted item's editor, and a long press steps back up one level. The list, in order: `BUZZER` (Off/End/Countdown), `COUNTDOWN` (countdown beep window, 0–30 s), `FINISH` (auto-clear/hold/re-alert), `AUTOCLEAR` (auto-clear delay, 1–300 s), `REALERT` (re-alert cadence, 5–300 s), `ICON` (show/hide the timer icon, ADR-0005), `BAR` (show/hide the progress bar), `MAIN` (back to the main menu). Inside an item's editor only the **bare value** is shown (e.g. `END`, `30`, `ON`): left/right cycles the enum, adjusts the number (step 5 for `AUTOCLEAR`/`REALERT`, step 1 for `COUNTDOWN` — see [ADR 0003](adr/0003-timer-tuning-knobs-on-device.md) for these magnitudes and why), or toggles the boolean (`ICON`/`BAR` — both buttons flip it); a short press confirms and returns to the list, and a long press saves and returns to the list. Mode changes (`BUZZER`/`FINISH`) apply and publish to MQTT/HA immediately as you scroll; all edits persist together when you leave the list (long-press out of the list, or select `MAIN`). See [ADR 0016](adr/0016-timer-menu-drillin-navigation.md). |
